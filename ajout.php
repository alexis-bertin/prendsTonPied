		...
		if (VerifSaisie()) {
			...
			if ((preg_match("/^\pL+(?>[- ']\pL+)*$/u", $nom)) && (preg_match("/^\pL+(?>[- ']\pL+)*$/u", $prenom)) && (filter_var($email, FILTER_VALIDATE_EMAIL)) && ((strlen($mdp) >= 8) && (preg_match('#^(?=.*[a-z])(?=.*[0-9])(?=.*\W)#', $mdp)))) {
				if ($email==$email2 && $mdp==$mdp2) {
					...
					try {
						$sql="INSERT INTO  Client VALUES (NULL, :nom, :prenom, :sexe, :email, :mdp, :tel, :date_naissance, :code_postal, :ville, :nom_rue, :num_rue, :num_appart)";
						$prep=$cx->prepare($sql);
						$tabVal=array(':nom'=>$nom, ':prenom'=>$prenom, ':sexe'=>$sexe, ':email'=>$email, ':mdp'=>$mdpc, ':tel'=>$tel, ':date_naissance'=>$date_naissance, ':code_postal'=>$cp, ':ville'=>$ville, ':nom_rue'=>$norue, ':num_rue'=>$nurue, ':num_appart'=>$appart, );
