...
<html>
	<head>
		<title>Prends ton pied - Recherche</title>
	...
	<body>
		...
		<?php
			include_once('connexions.php');
			if (isset($_SESSION['searchh']) && ($_POST['valider'] == "Valider")) {
				$var_search = $_SESSION['searchh'];
			}
			else {
				$var_search = $_POST['q'];
				$_SESSION['searchh'] = $var_search;
			}
			$occur = 0;
			$sql=$cx->prepare('SELECT marque, modele, type_course, type_foulee FROM Chaussures');
			$sql->execute();
			$ligne=$sql->fetchAll(PDO::FETCH_ASSOC);
			foreach ($ligne as $ligne2) {
				foreach ($ligne2 as $ligne33) {
					$ligne3 .= $ligne33;
				}
				if  (preg_match("/".$var_search."/i", $ligne3) != FALSE) {
					$occur += 1;
				} 
				$ligne3="";
			}
			if ($occur != 0) {
				?>
				<form method="POST" action="search.php" name="test1" >
					...
					<tr style="margin-top: 5px;">
						...
						<?php
							...
							try {
								...
								echo "<td style=\"background-color: grey; padding: 10px;\"><table><tr><td style=\"padding-top: 10px; \">";
								$res3=$cx->query($sql3);
				...
				<?php
				echo "<div style=\"margin-left: 5vw; margin-top: 5vw;\">";
				if ($_POST['valider'] == "Valider") {
					echo mb_strtoupper("<b>Résultats de recherche : ".$var_search."</b>", 'UTF-8');
				}
				else {
					echo mb_strtoupper("<b>Résultats de recherche : ".$var_search."</b> (".($occur).")", 'UTF-8');
				}
				echo "</div><br/>";
				//Appel de la BDD
				include_once('connexions.php');
				...
				if ($_POST['zbeub'] == "promos") {
					$sql = "SELECT DISTINCT img_chaussure,marque,modele,prix,promotion,Chaussures.id_chaussure FROM Chaussures JOIN Stock ON Chaussures.id_Chaussure = Stock.id_chaussure WHERE (type_course LIKE '%".$_POST['t_course']."%') AND (marque LIKE '".implode("%' OR marque  LIKE '", $_POST["options"])."%') AND (pointure LIKE '".implode("%' OR pointure  LIKE '", $_POST["options2"])."%') AND (prix BETWEEN '".$_POST["prixmini"]."' AND '". $_POST["prixmaxi"]."') AND (promotion != 0) AND ((marque LIKE '". $var_search."%') OR (modele LIKE '%". $var_search."%') OR (type_foulee LIKE '". $var_search."%') OR (type_course LIKE '". $var_search."%')) ORDER BY promotion DESC" ;
				}
				elseif ($_POST['zbeub'] == "nouveau") {
					$sql = "SELECT DISTINCT img_chaussure,marque,modele,prix,promotion,Chaussures.id_chaussure FROM Chaussures JOIN Stock ON Chaussures.id_Chaussure = Stock.id_chaussure WHERE (type_course LIKE '%".$_POST['t_course']."%') AND (marque LIKE '".implode("%' OR marque  LIKE '", $_POST["options"])."%') AND (pointure LIKE '".implode("%' OR pointure  LIKE '", $_POST["options2"])."%') AND (prix BETWEEN '".$_POST["prixmini"]."' AND '". $_POST["prixmaxi"]."') AND ((marque LIKE '". $var_search."%') OR (modele LIKE '%". $var_search."%') OR (type_foulee LIKE '". $var_search."%') OR (type_course LIKE '". $var_search."%')) ORDER BY date_depot DESC";
				}
				elseif ($_POST['zbeub'] == "popu") {
					$sql = "SELECT DISTINCT img_chaussure,marque,modele,prix,promotion,Chaussures.id_chaussure FROM Chaussures JOIN Stock ON Chaussures.id_Chaussure = Stock.id_chaussure WHERE (type_course LIKE '%".$_POST['t_course']."%') AND (marque LIKE '".implode("%' OR marque  LIKE '", $_POST["options"])."%') AND (pointure LIKE '".implode("%' OR pointure  LIKE '", $_POST["options2"])."%') AND (prix BETWEEN '".$_POST["prixmini"]."' AND '". $_POST["prixmaxi"]."') AND ((marque LIKE '". $var_search."%') OR (modele LIKE '%". $var_search."%') OR (type_foulee LIKE '". $var_search."%') OR (type_course LIKE '". $var_search."%')) ORDER BY nb_achat_C DESC" ;
				}
				elseif (($_POST['prixmini'] != NULL) || ($_POST['prixmaxi'] != NULL)) {
					$sql = "SELECT DISTINCT img_chaussure,marque,modele,prix,promotion,Chaussures.id_chaussure FROM Chaussures JOIN Stock ON Chaussures.id_Chaussure = Stock.id_chaussure WHERE (type_course LIKE '%".$_POST['t_course']."%') AND (marque LIKE '".implode("%' OR marque  LIKE '", $_POST["options"])."%') AND (pointure LIKE '".implode("%' OR pointure  LIKE '", $_POST["options2"])."%') AND (prix BETWEEN '".$_POST["prixmini"]."' AND '". $_POST["prixmaxi"]."') AND ((marque LIKE '". $var_search."%') OR (modele LIKE '%". $var_search."%') OR (type_foulee LIKE '". $var_search."%') OR (type_course LIKE '". $var_search."%'))";
				}
				else {
					$sql = "SELECT DISTINCT img_chaussure,marque,modele,prix,promotion,Chaussures.id_chaussure FROM Chaussures JOIN Stock ON Chaussures.id_Chaussure = Stock.id_chaussure WHERE (type_course LIKE '%".$_POST['t_course']."%') AND (marque LIKE '".implode("%' OR marque  LIKE '", $_POST["options"])."%') AND (pointure LIKE '".implode("%' OR pointure  LIKE '", $_POST["options2"])."%') AND ((marque LIKE '". $var_search."%') OR (modele LIKE '%". $var_search."%') OR (type_foulee LIKE '". $var_search."%') OR (type_course LIKE '". $var_search."%'))"; 
				}
				try {
					$res=$cx->query($sql);
				...
				echo "<div style=\"text-align: center; margin-top: 5vw;\">";
				echo mb_strtoupper("Désolé, nous n'avons trouvé aucun résultat pour <b>\"".$var_search."\"</b>", 'UTF-8');
				echo "</div>";
				echo " ";
				echo "<br/><div style=\"text-align:center;\"><form style=\"display: inline-block;\" method=\"POST\" action=\"search.php\" name=\"rechercher1\" role=\"search\"><input type=\"search\" id=\"maRecherche\" name=\"q\" placeholder=\"Rechercher encore\" size=\"30\"></form>";
			}
		?>
